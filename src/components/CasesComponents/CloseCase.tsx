...
export default function CloseCase({ closeCase,reloadActive }: any) {
    ...
    >
      ...
      {isMobileView ? (
        <div className="mobile_div">
          {closeCase &&
            closeCase.map((item: any, index: number) => (
      ...
      {isModalOpen && (
          <div className="formToTask3">
            <EditCloseCaseModal id={selectedItemId} closeModal={closeModal} reloadActive={reloadActive} />
          </div>
      )}
    </motion.div>
  );
